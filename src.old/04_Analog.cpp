...
void analogSetup()
{
  myRotation.Current.mask = 7;
  myTranslation.Current.mask = 7;
  myWheel.Current.mask = 7;
}
...
void readRotation()
{
  ...
  if (Rotation_Throttle_Con)
  {
    ...
    if (!Throttle_Con)
    {
      ...
      {
        Rotation_throttle.Current = 0;
      }
      else if (throttle_mm == maxVolt)
      {
        Rotation_throttle.Current = INT16_MAX;
      }
      else
      {
        Rotation_throttle.Current = map(throttle_mm, minVolt_, maxVolt_, 32, INT16_MAX - 32);
      }
    }
  }
}
...
void readThrottle()
{
  ...
  {
    throttle.Current = max_send_throttle;
  }
  else
  {
    throttle.Current = map(throttle_mm, min_throttle, max_throttle, 0, max_send_throttle);
  }
}
